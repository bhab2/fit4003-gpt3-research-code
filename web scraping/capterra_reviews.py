...
try:
    for parent in correct_result.parents:
        if parent.name == 'a':
            ...
            searched_app_link = searched_app_link.replace(
                'software', 'reviews')  # change link to reviews link
...
with open(f'{fileName}.csv', 'w', encoding='UTF8') as f:
    ...
    for page in range(1, 100):
        ...
        app_html_text = requests.get(
            searched_app_link + PAGE_URL + str(page)).text
        ...
        if reviews:
            for index, review in enumerate(reviews):
                ...
                review_date = review.find('span', class_='ms-2').text.strip()
                # get the review title
                review_title = review.find(
                    'h3', class_='h5 fw-bold').text.strip()
                review_summary = None
                review_comments = []
                ...
                writer.writerow([review_date, review_title,
                                review_summary, review_comments])
